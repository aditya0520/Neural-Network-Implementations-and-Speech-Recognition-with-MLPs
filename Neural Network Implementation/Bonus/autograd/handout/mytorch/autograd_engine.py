...
class Autograd:
    ...
    def add_operation(self,
        ...
        for input_var in inputs:
            if not self.gradient_buffer.is_in_memory(input_var):
                self.gradient_buffer.add_spot(input_var)


        # TODO: Append an Operation object to the self.operation_list
        operation = Operation(inputs=inputs, output=output, gradients_to_update=gradients_to_update, backward_operation=backward_operation)
    ...
    def backward(self, divergence):
        ...
        gradient_to_propagate = np.ones_like(self.operation_list[-1].output) * divergence
        print("\nStarting Backward Pass")
        for idx, operation in enumerate(reversed(self.operation_list)):
            print(f"\nOperation {len(self.operation_list) - idx}: {operation}")
            print(f"Gradient to propagate before this operation: {gradient_to_propagate}")

            
            input_gradients = operation.backward_operation(gradient_to_propagate, *operation.inputs)
            print(f"Input Gradients: {input_gradients}")

            for i, input_var in enumerate(operation.inputs):
                gradient_for_input = input_gradients[i]

                if operation.gradients_to_update[i] is not None:
                    print(f"Updating gradient for parameter: {operation.gradients_to_update[i]}")
                    operation.gradients_to_update[i] += gradient_for_input

                else:

                    if self.gradient_buffer.is_in_memory(input_var):
                        print(f"Updating gradient for input: {input_var}")
                        self.gradient_buffer.update_param(input_var, gradient_for_input)

                    else:
                        self.gradient_buffer.add_spot(input_var)
                        print(f"Adding new spot for input: {input_var}")
                        self.gradient_buffer.update_param(input_var, gradient_for_input)
            
            gradient_to_propagate = input_gradients[0]
            print(f"Gradient to propagate after this operation: {gradient_to_propagate}")
